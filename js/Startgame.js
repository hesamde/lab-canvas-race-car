const gameApp = {
    ...
    setEventListeners() {
        ...
        const  {key} = event
            if (key === 'ArrowLeft') {this.carSpecs.pos.x -= 35}
            if (key === 'ArrowRight') {this.carSpecs.pos.x += 35}
            if (key == 'ArrowUp') {this.carSpecs.pos.y -= 35}
            if (key == 'ArrowDown') {this.carSpecs.pos.y += 35}
        }
    },
